...
function getCustomerNames(){
    ...
    include('dao/connect.php');
        $result = $db->query("select * from customer where cust_id ='$customer_id'");
        $name = "";
         while ($de = $result->fetch_assoc()) {
             $name = $de['fname']." ".$de['mname'];
         }
         return $name;
}
...
//Page header
    function Header() {
        ...
        $this->Cell(195, 6, "". strtoupper(getCustomerNames())." FROM ".$start." to ".$end , 0, 0, "C", 0);
...
$pdf->Cell(10, 7, "#", 1, 0, "L", 1);
       ...
       $pdf->Cell(65, 7, "Sale Date", 1, 0, "C", 1);
       $pdf->Cell(35, 7, "Total Amount", 1, 0, "C", 1);
       $pdf->Cell(35, 7, "Paid Amount", 1, 0, "C", 1);
...
if(isset($_GET['from'])){
        ...
        $statement = "SELECT `fname`,`mname`, DATE_FORMAT(sales_date,\" %d-%m-%Y \") as saledate, SUM(`total_amount`) AS total, SUM(`amount_paid`) AS paid, SUM(total_amount - amount_paid) AS balance ,sales_date
FROM `customer` JOIN `sales` ON customer.`cust_id` = sales.`customer_id`
WHERE  sales_date >= '$from' AND sales_date <= '$to' AND customer.`cust_id` = '$customer_id'
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Cell(65, 7, $row['saledate'], 1, 0, "L", $fill);
