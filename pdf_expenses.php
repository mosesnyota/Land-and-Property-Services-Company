...
//Page header
    function Header() {
        ...
        $this->Cell(195, 6, "EXPENSES REPORT FOR THE PERIOD BETWEEN ".$start." AND ".$end , 0, 0, "C", 0);
...
if(isset($_GET['from'])){
 ...
 $statement = "SELECT expenses.*, `expense_types`.`expense_type` FROM expenses JOIN `expense_types` ON `expenses`.`expense_type` = `expense_types`.`expense_type_id` "
                    . " WHERE  expense_date >= '$start' AND expense_date <= '$end' order by expense_date desc";
...
$pdf->Cell(155, 7, "Total Expenses", 1, 0, "C", 1);
