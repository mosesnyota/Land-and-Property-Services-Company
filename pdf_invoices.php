...
//Page header
    function Header() {
        ...
        $this->Cell(195, 6, "Unpaid Invoices" , 0, 0, "C", 0);
...
$pdf->Cell(10, 7, "#", 1, 0, "L", 1);
       ...
       $pdf->Cell(40, 7, "Supplier", 1, 0, "C", 1);
       $pdf->Cell(40, 7, "Contact Person", 1, 0, "C", 1);
       $pdf->Cell(25, 7, "Invoice Date", 1, 0, "C", 1);
...
$pdf->setXY($x, $y);
 ...
 $statement = "SELECT `company_name`,`contact_person`, invoice.*,DATE_FORMAT(invoice.`invoice_date`,'%d-%m-%Y') AS sdate  FROM invoice JOIN `suppliers` ON invoice.`supplier_id` = suppliers.`supplier_id` where pay_status='Unpaid' ORDER BY `invoice_date` DESC ";
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Cell(40, 7, strtoupper($row['company_name']), 1, 0, "L", $fill);
    $pdf->Cell(40, 7, strtoupper($row['contact_person']), 1, 0, "L", $fill);
    $pdf->Cell(25, 7, $row['sdate'], 1, 0, "R", $fill);
    $pdf->Cell(25, 7, $row['amount'], 1, 0, "R", $fill);
    $pdf->Cell(25, 7, $row['paid_amount'], 1, 0, "R", $fill);
    $pdf->Cell(25, 7, $row['amount'] - $row['paid_amount'], 1, 0, "R", $fill);
    ...
    if ($y > 275) {
       ...
       $pdf->Cell(10, 7, "#", 1, 0, "L", 1);
        $pdf->Cell(50, 7, "Supplier", 1, 0, "C", 1);
       $pdf->Cell(50, 7, "Contact Person", 1, 0, "C", 1);
